...
export class SavedArtworkRepositoryImpl implements ISavedArtworkRepository {
  ...
  async saveArtwork(artwork: Artwork): Promise<void> {
    try {
      ...
      // Save updated list
      await localStorageService.setItem(this.STORAGE_KEY, [...savedArtworks, artworkToSave])
    } catch (error) {
  ...
  async getAllSavedArtworks(): Promise<SavedArtwork[]> {
    try {
      const savedArtworks = await localStorageService.getItem<SavedArtwork[]>(this.STORAGE_KEY)
